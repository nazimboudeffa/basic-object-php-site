 ...
 * @return mix data
 */
function bobjps_call_hook($hook, $type, $params = null, $returnvalue = null) {
	global $Bobjps;
	$hooks = array();
	if (isset($Bobjps->hooks[$hook][$type])) {
 ...
 * @return bool
 */
function bobjps_trigger_callback($event, $type, $params = null) {
	global $Bobjps;
	$events = array();
	if (isset($Bobjps->events[$event][$type])) {
		$events[] = $Bobjps->events[$event][$type];
	}
